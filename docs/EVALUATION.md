...
6. Gate chuan: `PYTHONPATH=. python scripts/run-eval.py --seed 42 --backend auto` → PASS khi
   OOS Sharpe>0.5, DSR>0.95 (K=so config da thu), x2>0, x5>-1.0, maxDD>-15%.
   `auto` prefers the Rust backtest backend when the release binary is present;
   `--backend python` keeps the reference path and `--backend rust` is fail-closed.
