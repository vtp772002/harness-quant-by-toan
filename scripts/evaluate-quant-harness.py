...
def main() -> int:
    ...
    cases = [
        ...
        case("rust-core-build", "evaluation",
             "cargo build --release --manifest-path crates/quant-core/Cargo.toml", True,
             lambda: _run_ok(["cargo", "build", "--release", "--manifest-path",
                              "crates/quant-core/Cargo.toml"])),
        case("gate", "evaluation", f"scripts/run-eval.py --seed {a.seed} --backend rust", True,
